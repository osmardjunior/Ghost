...
const post_revisions_1 = require("../../../../core/server/lib/post-revisions");
const config = {
    max_revisions: 10,
    revision_interval_ms: 1000
};
function makePostLike(data = {}) {
...
describe('PostRevisions', function () {
    describe('shouldGenerateRevision', function () {
        it('should return true if there are no revisions', function () {
            const postRevisions = new post_revisions_1.PostRevisions({ config, model: {} });
    ...
    describe('getRevisions', function () {
        ...
        it('returns one revision when there are no existing revisions', async function () {
            ...
            strict_1.default.equal(actual.length, 1);
            strict_1.default.equal(actual[0].lexical, 'current');
            strict_1.default.equal(actual[0].author_id, '123');
            strict_1.default.equal(actual[0].title, 'foo bar baz');
        });
        it('does not limit the number of revisions if under the max_revisions count', async function () {
            const postRevisions = new post_revisions_1.PostRevisions({
                config: {
                    max_revisions: 2,
            ...
            strict_1.default.equal(actual.length, 2);
        });
    ...
    describe('removeAuthor', function () {
        it('removes the provided author from post revisions', async function () {
            ...
            const modelStub = {
                ...
                bulkEdit: sinon_1.default.stub().resolves()
            };
            const postRevisions = new post_revisions_1.PostRevisions({
                config,
                model: modelStub
            ...
            sinon_1.default.assert.calledOnce(modelStub.bulkEdit);
            const bulkEditArgs = modelStub.bulkEdit.getCall(0).args;
            strict_1.default.deepEqual(bulkEditArgs[0], ['revision123', 'revision456', 'revision789']);
            strict_1.default.equal(bulkEditArgs[1], 'post_revisions');
            strict_1.default.deepEqual(bulkEditArgs[2], {
                data: {
                    author_id: null
                },
                column: 'id',
        ...
        it('does nothing if there are no post revisions by the provided author', async function () {
            ...
            sinon_1.default.assert.notCalled(modelStub.bulkEdit);
        });
    });
});
