"use strict";
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
const strict_1 = __importDefault(require("assert/strict"));
const identity_token_service_1 = require("../../../../../core/server/services/identity-tokens/identity-token-service");
const node_jose_1 = require("node-jose");
const jsonwebtoken_1 = require("jsonwebtoken");
describe('IdentityTokenService', function () {
    it('Can create JWTs', async function () {
        const privateKey = `-----BEGIN PRIVATE KEY-----

-----END PRIVATE KEY-----`;
        const issuer = 'issuer.com';
        const keyStore = node_jose_1.JWK.createKeyStore();
        const key = await keyStore.add(privateKey, 'pem');
        const service = new identity_token_service_1.IdentityTokenService(privateKey, issuer, key.kid);
        const token = await service.getTokenForUser('egg@ghost.org', 'Legend');
        const claims = (0, jsonwebtoken_1.verify)(token, key.toPEM());
        if (typeof claims === 'string') {
            throw new Error('Unexpected return type');
        }
        strict_1.default.equal(claims.sub, 'egg@ghost.org');
        strict_1.default.equal(claims.role, 'Legend');
        strict_1.default.equal(claims.iss, 'issuer.com');
    });
});
