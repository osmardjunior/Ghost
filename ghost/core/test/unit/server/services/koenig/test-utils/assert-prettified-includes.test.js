...
const assert_prettified_includes_1 = require("./assert-prettified-includes");
describe('assertPrettifiedIncludes', function () {
    it('passes when HTML includes expected content', function () {
        ...
        // Should not throw
        (0, assert_prettified_includes_1.assertPrettifiedIncludes)(actual, expected);
    });
    ...
    it('fails with helpful error message', function () {
        const actual = '<div><p>Hello World</p></div>';
        const expected = '<p>Goodbye World</p>';
        strict_1.default.throws(() => (0, assert_prettified_includes_1.assertPrettifiedIncludes)(actual, expected), (error) => {
            (0, strict_1.default)(error instanceof Error);
            strict_1.default.ok(error.message.includes('Received:'));
            strict_1.default.ok(error.message.includes('Expected:'));
            strict_1.default.ok(error.message.includes(actual));
            strict_1.default.ok(error.message.includes(expected));
            return true;
        });
    });
