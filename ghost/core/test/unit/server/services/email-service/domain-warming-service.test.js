...
const domain_warming_service_1 = require("../../../../../core/server/services/email-service/domain-warming-service");
...
describe('Domain Warming Service', function () {
    ...
    beforeEach(function () {
        ...
        Email = (0, utils_1.createModelClass)({
            findAll: []
        });
        // Fix the current time for consistent test results
        clock = sinon_1.default.useFakeTimers(new Date('2024-01-15T12:00:00Z').getTime());
    });
    afterEach(function () {
        clock.restore();
    ...
    describe('constructor', function () {
        it('should instantiate with required dependencies', function () {
            const service = new domain_warming_service_1.DomainWarmingService({
                models: { Email },
                config
            });
    ...
    describe('getWarmupLimit', function () {
        ...
        it('should return 200 (start value) when no previous emails exist (day 0)', async function () {
            ...
            strict_1.default.equal(result, 200);
        });
        ...
        it('should return emailCount when it is less than calculated limit', async function () {
            ...
            const result = await service.getWarmupLimit(1000);
            strict_1.default.equal(result, 1000);
        });
        it('should return calculated limit when emailCount is greater', async function () {
            ...
            strict_1.default.equal(result, 237);
        });
        ...
        it('should query for first email with csd_email_count', async function () {
            const findPageStub = sinon_1.default.stub().resolves({ data: [] });
            Email = {
            ...
            sinon_1.default.assert.calledOnce(findPageStub);
            const callArgs = findPageStub.firstCall.args[0];
            strict_1.default.ok(callArgs.filter);
            strict_1.default.ok(callArgs.filter.includes('csd_email_count:-null'));
            strict_1.default.equal(callArgs.order, 'created_at ASC');
            strict_1.default.equal(callArgs.limit, 1);
        });
        it('should return correct warmup progression through the days', async function () {
            ...
            for (const testCase of testCases) {
                const EmailModel = (0, utils_1.createModelClass)({
                    findAll: [{
                            csd_email_count: 100,
                ...
                strict_1.default.equal(result, testCase.expected, `Expected ${testCase.expected} for day ${testCase.day}, but got ${result}`);
            }
        });
        it('should return emailCount after warmup period is complete', async function () {
            ...
            strict_1.default.equal(result, 1000000);
        });
