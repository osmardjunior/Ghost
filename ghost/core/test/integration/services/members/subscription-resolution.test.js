...
async function createMemberWithCustomer(email) {
...
async function addSubscription(email, overrides) {
...
async function resolvedSubscriptionId(memberId) {
...
describe('Subscription resolution priority', function () {
    ...
    it('picks an active subscription over a cancelled one regardless of recency', async function () {
        ...
        strict_1.default.equal(resolved, olderActive.get('id'));
        strict_1.default.notEqual(resolved, recentCancelled.get('id'));
    });
    it('picks the most recent active subscription on upgrade (Bronze → Silver)', async function () {
        ...
        const resolved = await resolvedSubscriptionId(member.get('id'));
        strict_1.default.equal(resolved, newSilver.get('id'));
        strict_1.default.notEqual(resolved, oldBronze.get('id'));
    });
    it('picks the most recent active subscription on downgrade (Silver → Bronze)', async function () {
        ...
        const resolved = await resolvedSubscriptionId(member.get('id'));
        strict_1.default.equal(resolved, newBronze.get('id'));
        strict_1.default.notEqual(resolved, oldSilver.get('id'));
    });
    it('treats trialing, past_due, and unpaid as active alongside active', async function () {
        // Each of these statuses counts as "active" for resolution and must beat
        // a more recent canceled sub. The title named all three but only trialing
        // was exercised — cover each so a regression on past_due/unpaid is caught.
        for (const activeStatus of ['trialing', 'past_due', 'unpaid']) {
            const email = `active-equivalent-${activeStatus}@test.member`;
            const member = await createMemberWithCustomer(email);
            const oldCancelled = await addSubscription(email, {
                subscription_id: `${activeStatus}_old_cancelled`,
                status: 'canceled',
                created_at: daysAgo(40)
            });
            const recentActiveEquivalent = await addSubscription(email, {
                subscription_id: `${activeStatus}_recent`,
                status: activeStatus,
                created_at: daysAgo(3)
            });
            const resolved = await resolvedSubscriptionId(member.get('id'));
            strict_1.default.equal(resolved, recentActiveEquivalent.get('id'), `${activeStatus} sub should resolve as current over a more recent canceled sub`);
            strict_1.default.notEqual(resolved, oldCancelled.get('id'));
        }
    });
    it('excludes incomplete subscriptions from resolution — only sub is incomplete', async function () {
        ...
        strict_1.default.equal(await resolvedSubscriptionId(member.get('id')), null);
    });
    ...
    it('ranks by created_at, not start_date — older sub with future start_date does not out-rank newer sub', async function () {
        ...
        const resolved = await resolvedSubscriptionId(member.get('id'));
        strict_1.default.equal(resolved, newerInEffect.get('id'));
        strict_1.default.notEqual(resolved, earlierWithFutureStart.get('id'));
    });
    it('resolves to the only subscription when a member has just one', async function () {
        ...
        const resolved = await resolvedSubscriptionId(member.get('id'));
        strict_1.default.equal(resolved, onlySub.get('id'));
    });
    it('breaks identical-created_at ties deterministically by subscription id', async function () {
        ...
        const resolved = await resolvedSubscriptionId(member.get('id'));
        strict_1.default.equal(resolved, lowerId);
    });
    ...
    it('does not include comped members (no Stripe subscription) in subscription filters', async function () {
        ...
        strict_1.default.equal(await resolvedSubscriptionId(compedMember.get('id')), null);
        ...
        strict_1.default.equal(activeMembers.data.find((m) => m.id === compedMember.get('id')), undefined, 'comped member must not appear in subscriptions.status:active');
    });
});
